  ...
  alloc {
    SynthDef(\Asterion, {
      arg amp = 1, attack = 0.1, bandstop_hz = 50, bandstop_width = 0.5,
	    ...
	    var mid = SinOsc.ar(osc_hz, LinRand(5,15.0), osc_amp * {SinOsc.kr(1).range(0.75, 1)}, {SinOsc.kr(22).range(1, 2)});
	    var high = SinOsc.ar(osc_hz * 2, LinRand(15,25.0), osc_amp * {SinOsc.kr(1).range(0.75, 1)}, {SinOsc.kr(22).range(1, shimmer * 0.1)});
      var noise = LPF.ar({BrownNoise.ar(noise_amp)}, {SinOsc.kr(45).range(noise_cutoff - gloom, noise_cutoff)});
      var mix = Splay.ar([low, mid, high, noise]);
